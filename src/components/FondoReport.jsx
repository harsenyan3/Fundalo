    ...
    <div className={styles.barRow}>
      <div className={styles.barBlocks}>
        {Array.from({ length: 10 }).map((_, i) => (
          <div key={i} className={styles.barBlock} style={{ background: i < filled ? color : '#e2e8f0' }} />
        ))}
      </div>
...
export default function FondoReport({ profile, report, classified, lang, onBack }) {
  ...
  const scoreRows = [
    { label: isEs ? 'Consistencia de ingresos' : 'Revenue consistency', val: report.scoreBreakdown?.revenueConsistency || 0, max: 30 },
    { label: isEs ? 'Salud del flujo de caja' : 'Cash flow health', val: report.scoreBreakdown?.cashFlowHealth || 0, max: 25 },
    { label: isEs ? 'Separación negocio/personal' : 'Business vs personal separation', val: report.scoreBreakdown?.separationDiscipline || 0, max: 20 },
    { label: isEs ? 'Diversidad de clientes' : 'Client diversity', val: report.scoreBreakdown?.clientDiversity || 0, max: 10 },
    { label: isEs ? 'Historial operativo' : 'Operating history', val: report.scoreBreakdown?.operatingHistory || 0, max: 5 },
    { label: isEs ? 'Calidad de evidencia' : 'Evidence quality', val: report.scoreBreakdown?.evidenceQuality || 0, max: 10 },
  ]
    ...
    <div className={styles.page}>
      <div className={styles.controls}>
        ...
        <div className={styles.viewToggle}>
          <button className={`${styles.viewBtn} ${view === 'owner' ? styles.viewActive : ''}`} onClick={() => setView('owner')}>
            {isEs ? 'Mi vista' : 'Owner view'}
          </button>
          <button className={`${styles.viewBtn} ${view === 'bank' ? styles.viewBankActive : ''}`} onClick={() => setView('bank')}>
      ...
      <div className={`${styles.report} ${view === 'bank' ? styles.reportBank : ''}`} id="printable">
        <div className={styles.reportHeader}>
          ...
          <div className={styles.headerRight}>
            <div className={styles.metaRow}><span className={styles.metaLabel}>{isEs ? 'Fecha' : 'Date'}</span><span className={styles.metaVal}>{new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span></div>
            ...
            <div className={styles.metaRow}><span className={styles.metaLabel}>{isEs ? 'Dueño' : 'Owner'}</span><span className={styles.metaVal}>{profile.ownerName || 'N/A'}</span></div>
        ...
        <div className={styles.ratingSection} style={{ background: rating.bg, borderColor: rating.border }}>
          <div className={styles.ratingLeft}>
            ...
            <div className={styles.ratingDesc}>{isEs ? 'Calificación Fundalo' : 'Fundalo rating'}</div>
          </div>
        ...
        <div className={styles.section}>
          <div className={styles.sectionTitle}>{view === 'bank' ? 'EXECUTIVE ASSESSMENT' : (isEs ? 'EVALUACIÓN EJECUTIVA' : 'EXECUTIVE ASSESSMENT')}</div>
        ...
        <div className={styles.twoCol}>
          <div className={styles.section}>
            <div className={styles.sectionTitle}>{isEs ? 'DESGLOSE DEL PUNTAJE' : 'SCORE BREAKDOWN'}</div>
